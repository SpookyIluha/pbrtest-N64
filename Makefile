...
PBM_LIST = $(shell find $(ASSETS_DIR)/textures/ -type f -name '*.pbm')
...
N64_C_AND_CXX_FLAGS += -I $(SOURCE_DIR) -O3 -ftrivial-auto-var-init=zero
...
$(FILESYSTEM_DIR)/textures/%.pbm: $(ASSETS_DIR)/textures/%.pbm
	@mkdir -p $(dir $@)
	@echo "    [PBM] $@"
	$(N64_BINDIR)/mkasset -c 0 -o $(dir $@) "$<"

$(BUILD_DIR)/$(NAME).dfs: $(ASSETS_LIST)
$(BUILD_DIR)/$(NAME).elf: $(SRCS:$(SOURCE_DIR)/%.c=$(BUILD_DIR)/%.o) $(SRCS:$(SOURCE_DIR)/%.cpp=$(BUILD_DIR)/%.o)  $(ASM:$(SOURCE_DIR)/%.S=$(BUILD_DIR)/%.o)
